...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x, content_features, style_features):
        out = self.conv1(x)
        out = self.adain(out, content_features, style_features)
        out = self.conv2(out)
        out = self.adain(out, content_features, style_features)
        return x + out
...
class Generator(nn.Module):
    def __init__(self, adain, img_channels, num_features=64, num_residuals=9):
        ...
        self.up_blocks = nn.ModuleList(
            [
                ConvBlock(num_features * 4, num_features * 2, down=False, kernel_size=3, stride=2, padding=1, output_padding=1, use_in=True),
                ConvBlock(num_features * 2, num_features, down=False, kernel_size=3, stride=2, padding=1, output_padding=1, use_in=True),
            ]
        )
        self.last = nn.Conv2d(
            num_features,
